 ...
 * Client-side module.
 * <p>
 * Client interact and receive messages from HOST and submit to database
...
public class App extends AbstractVerticle {
    ...
    public void start() {
        ...
        client.publishHandler(message -> submitData(message.payload().toString()));

        /*
         * WAIT for a while and cease
         */
    ...
    @Override
    public void stop() {
        client.disconnect(d -> LOGGER.info(">> Disconnected"));
        mongoClient.close();
    }
    ...
    private void submitData(String data) {
        ...
        mongoClient.insert("random-data", jsonObject, res -> {
            if (res.succeeded()) {
                LOGGER.info("Submitted: " + res.result() + " | " + jsonObject);
            } else {
                LOGGER.info("Failed to submit data in DB", res.cause());
            }
        });
    }
}
